...
func (p *Parser) renderOutput(o io.Writer, packageName string, templateName string, opt *BlipOptions) {
	...
	p.wStr(o, fmt.Sprintf("// Generated by Blip\n"))
...
func (p *Parser) writeBody(node ast, depth int, o io.Writer) {
	...
	for _, ast := range node.GetChildren() {
		...
		case NODE_CONTENT:
			// var f1 = func() {
			cnum += 1
			p.wStr(o, fmt.Sprintf("%svar contentF%dS%d = func() (terror error) {\n", tabs, p.includeDepth, cnum))
		...
		case NODE_CONTENT:
			...
			p.wStr(o, fmt.Sprintf("\"%s\", contentF%dS%d)", p.trimAll(base.token.Literal), p.includeDepth, cnum))
